...
fn mov_cube(
    ...
    pos: (usize, usize),
    cube: &Cube,
) -> Option<(Dir, CubeSide, (usize, usize))> {
    let side_map = match cube_side {
        CubeSide::Top => &cube.top,
        CubeSide::Bottom => &cube.bottom,
        CubeSide::Left => &cube.left,
    ...
    Some(
        mov_cube(adj_dir, d - 1, adj_side, (adj_y, adj_x), cube).unwrap_or((
            dir,
            cube_side,
            (y, x),
        )),
...
fn part2(map: &Array2<char>, instructions: &Vec<Instruction>) -> usize {
    ...
    let (side, cube) = if side == 4 {
        ...
        let right = flip_horizontal(
            rotate_ccw(map.slice(s![2 * side..3 * side, 3 * side..4 * side])).view(),
        );
        (
            side,
            Cube {
    ...
    for ins in instructions {
        match ins {
            ...
            Instruction::Move(d) => {
                (dir, cube_side, pos) = mov_cube(dir, *d, cube_side, pos, &cube).unwrap()
            }
        };
    }
...
mod test {
    ...
    #[test]
    fn test_real_input() {
        let input = std::fs::read_to_string(aoclib::get_test_input_file!(22)).unwrap();
        let (map, instr_str) = input.split_once("\n\n").unwrap();
        let map = aoclib::read_string_char_matrix(map.trim_end()).unwrap();
        let instructions = parse_instructions(instr_str.trim());
